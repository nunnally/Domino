...
export function GameForm({
  ...
  const activePlayers = useMemo(
    () =>
      [...players]
        ...
        .sort(
          (a, b) =>
            Number(isGuestPlayer(a)) - Number(isGuestPlayer(b)) ||
  ...
  const updateBonus = (
    ...
    if (bonus === "gabuada") {
      setGabuadaSlot(checked ? slot : null);
    } else {
      setSenaSlot(checked ? slot : null);
    }
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    const guestIds = new Set(
      activePlayers.filter(isGuestPlayer).map(({ id }) => id),
    );
  ...
  const select = (
    label: string,
    slot: Slot,
    bonusLabels: Array<"Gabuada" | "Sena">,
  ) => {
    ...
    const selectablePlayers = activePlayers.filter(
      (player) =>
        ...
        <div className="player-select-control">
          ...
          >
            ...
            <span className="player-picker-label">
              {selected?.name ?? "Escolher jogador"}
            </span>
          ...
          <select
            ...
            value={slots[slot]}
            onChange={(event) => choosePlayer(slot, event.target.value)}
            aria-label={label}
            tabIndex={-1}
          >
            ...
            >
              ...
              {selectablePlayers.map((player) => (
                <button
                  ...
                  type="button"
                  role="option"
                  aria-selected={player.id === selected?.id}
                  key={player.id}
                  onClick={() => choosePlayer(slot, player.id)}
                >
                  <PlayerAvatar
                    name={player.name}
        ...
        {selected && (
          <div className="bonus-toggles">
            {bonusLabels.map((bonusLabel) => {
              ...
              const checked =
                bonus === "gabuada" ? gabuadaSlot === slot : senaSlot === slot;
              ...
              return (
                <label className="bonus-toggle" key={bonusLabel}>
                  <input
                    ...
                    checked={checked}
                    onChange={(event) =>
                      updateBonus(slot, bonus, event.target.checked)
                    }
  ...
  if (mode === "live" && liveTeams) {
    return (
      <LiveGame
        players={players}
        teamA={liveTeams.teamA}
        teamB={liveTeams.teamB}
        locate={locate}
        onSave={onSave}
        onCancel={() => {
          setLiveTeams(null);
    ...
    return (
      <section className="page-wrap game-mode-page">
        ...
        <header className="form-heading game-mode-heading">
          <div>
            <p className="eyebrow">Preparar a mesa</p>

            <h1>Nova partida</h1>

            <p className="game-mode-intro">
              Jogue acompanhando cada rodada ou registre uma partida que já
              terminou.
            </p>
          </div>
        ...
        <div className="game-mode-grid">
          <button
            ...
            type="button"
            onClick={() => setMode("live-setup")}
          >
            <div className="game-mode-card-icon">
              <Zap size={32} strokeWidth={2.7} />
            </div>

            <span className="sticker sticker-yellow">Novo</span>

            <h2>Jogar ao vivo</h2>

            <p>
              Marque cada ponto enquanto a partida acontece e acompanhe viradas,
              Gabuidas e momentos especiais.
            </p>
          ...
          >
            <div className="game-mode-card-icon">
              <ClipboardPenLine size={32} strokeWidth={2.5} />
            </div>

            <span className="sticker sticker-violet">Súmula</span>

            <h2>Registrar resultado</h2>

            <p>
              A partida já terminou? Informe vencedores, perdedores e o placar
              final normalmente.
            </p>
  ...
  return (
    <section className="page-wrap game-form-page">
      ...
      <div className="form-heading">
        <div>
          <p className="eyebrow">Mais uma para o histórico</p>
      ...
      <form className="game-form" onSubmit={submit}>
        <section className="team-block winners-block">
          ...
          <div className="team-heading">
            <span className="sticker sticker-yellow">Vencedores</span>
        ...
        <section className="team-block losers-block">
          ...
          <div className="team-heading">
            <span className="sticker sticker-violet">Perdedores</span>
        ...
        {errors.players && (
          <p className="form-error full-error" role="alert">
            {errors.players}
          </p>
        )}
        ...
        {(errors.date || errors.score || saveError) && (
          <p className="form-error full-error" role="alert">
            {errors.date ?? errors.score ?? saveError}
          </p>
        )}
