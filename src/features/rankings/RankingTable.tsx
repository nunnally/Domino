  ...
  return (
    <div className="ranking-list" role="table" aria-label="Ranking individual">
      <div className={showStreaks ? 'ranking-head with-streaks' : 'ranking-head'} role="row">
        ...
        {showStreaks && <><span className="desktop-streak">Maior sequência de vitórias</span><span className="desktop-streak">Maior sequência de derrotas</span></>}
      </div>
      {visibleRows.map((row, index) => (
        <div className={`${index === 0 ? 'ranking-row leader' : 'ranking-row'}${showStreaks ? ' with-streaks' : ''}`} role="row" aria-label={`${row.name}, ${row.wins} vitórias, ${row.losses} derrotas, ${row.winRate}%`} key={row.playerId}>
          ...
          <span className="rank-player"><PlayerAvatar name={row.name} photoUrl={row.photoUrl} /><span><strong>{row.name}</strong>{showStreaks && <span className="mobile-streaks"><small><Flame size={13} /> Maior sequência de vitórias: {row.maxWinStreak}</small><small><Trash2 size={13} /> Maior sequência de derrotas: {row.maxLossStreak}</small></span>}</span></span>
