...
export function RankingTable({ rows, limit }: RankingTableProps) {
  ...
  return (
    <div className="ranking-list" role="table" aria-label="Ranking individual">
      ...
      {visibleRows.map((row, index) => (
        <div className={index === 0 ? 'ranking-row leader' : 'ranking-row'} role="row" aria-label={`${row.name}, ${row.wins} vitórias, ${row.losses} derrotas, ${row.winRate}%`} key={row.playerId}>
          <strong className="rank-number">{String(index + 1).padStart(2, '0')}</strong>
          <span className="rank-player"><PlayerAvatar name={row.name} photoUrl={row.photoUrl} /><strong>{row.name}</strong></span>
