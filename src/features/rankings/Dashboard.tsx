...
import { filterGamesByPeriod, getHeadToHeadStats, getIndividualStats, getPairStats } from '../../lib/stats'
...
export function Dashboard({ players, games, period, onPeriodChange, onShowRankings, onNewGame }: DashboardProps) {
  const periodGames = filterGamesByPeriod(games, period)
  const individuals = getIndividualStats(players, periodGames).filter(({ games: played }) => played > 0)
  ...
  const lantern = [...individuals].sort((a, b) => b.losses - a.losses || a.name.localeCompare(b.name, 'pt-BR'))[0]
  const mainRivalry = rivalries[0]

  if (!leader || !bestPair) {
    return (
      <section className="empty-state page-wrap">
        <DominoTile left={0} right={0} />
        <p className="eyebrow">A mesa está limpa</p>
        <h1>Cadastre a primeira partida.</h1>
        <button className="button button-primary" type="button" onClick={onNewGame}>Começar agora</button>
      </section>
    )
  }
  ...
  return (
    <div className="page-wrap dashboard-page">
      <section className="dashboard-intro">
        ...
        <div className="period-switch" aria-label="Período do ranking">
          {(Object.keys(periodLabels) as PeriodFilter[]).map((value) => (
            <button type="button" className={period === value ? 'period-chip active' : 'period-chip'} key={value} onClick={() => onPeriodChange(value)}>
              {periodLabels[value]}
            </button>
          ))}
        </div>
      ...
      <section className="score-strip" aria-label="Resumo do período">
        <div><strong>{periodGames.length}</strong><span>partidas</span></div>
        <div><strong>{individuals.length}</strong><span>jogadores</span></div>
        <div><strong>{pairs.length}</strong><span>duplas</span></div>
        <div><strong>{rivalries.length}</strong><span>confrontos</span></div>
      </section>
      ...
      <div className="dashboard-grid">
        ...
        <aside className="side-stack">
          <section className="panel pair-card">
            ...
            <h3>{bestPair.names[0]}<br /><span>&</span> {bestPair.names[1]}</h3>
            <p><strong>{bestPair.wins} vitórias</strong> em {bestPair.games} jogos</p>
            <DominoTile left={bestPair.wins % 7} right={bestPair.losses % 7} className="pair-domino" />
          </section>
      ...
      {mainRivalry && (
        <section className="rivalry-banner">
          <div>
            ...
            <h2>{mainRivalry.pairLabels[0]} <span>vs.</span> {mainRivalry.pairLabels[1]}</h2>
          </div>
          <div className="rivalry-score"><strong>{mainRivalry.leaderWins}</strong><span>×</span><strong>{mainRivalry.trailerWins}</strong></div>
